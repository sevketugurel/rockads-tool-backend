...
class GeminiTranslationService(TranslationService):
    ...
    def _parse_translation_response(
        ...
        try:
            ...
            if "segments" in result:
                for seg_data in result["segments"]:
                    segment = TranslationSegment(
                        ...
                        confidence_score=seg_data.get("confidence_score", 0.8),
            ...
            if "cultural_adaptation" in result:
                ca_data = result["cultural_adaptation"]
                cultural_adaptation = CulturalAdaptation(
                    original_concept=ca_data.get("original_concept", ""),
                    adapted_concept=ca_data.get("adapted_concept", ""),
                    changes_made=ca_data.get("changes_made", []),
                    cultural_reasoning=ca_data.get("cultural_reasoning", ""),
                    risk_assessment=ca_data.get("risk_assessment", ""),
                    effectiveness_score=ca_data.get("effectiveness_score", 0.8)
                )

            return {
                "segments": segments,
                "full_text": result.get("full_translated_text", ""),
            ...
            return {
                "segments": [],
                "full_text": response_text[:500],  # Fallback to raw text
                "cultural_adaptation": None,
                "confidence": 0.5
            }
